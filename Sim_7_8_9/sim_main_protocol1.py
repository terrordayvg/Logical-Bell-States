...
sys.path.insert(1, 'Insert here the path')
...
def test_make_bacon_shor_xx_lattice_surgery_circuit(width: int, height: int, basis: str, rounds: int,noiset: int):
    ...
    circuit = gen.NoiseModel.uniform_depolarizing(noiset).noisy_circuit(circuit)
...
def Scheduler2_re(nfails,a,j,tf,dist): #i: number of cycles in QMEM, j is the sequential data identifier



    #Frequency of the source
    f=33 #33 MHz
    t1=1/(f) #time units are in $\mu s$
    ...
    bgen=t1#tH+tCX #bell state generation frequency
    #print(bgen, t1)
    bt=bgen#+ttrav #time if something bad occurs to redo bell and send through the distance d

    #Surface code
    #t2cyc=2*tH+4*tCX+tM
    t2cyc=(tM+2*tCX+2*tH)*2+(tM+2*tCX)*2



    tqec=a*t2cyc+tM #in memory ith QEC 
    tme_s=t2cyc+t2cyc*3+t2cyc+ttrav #merge+split time 



    #Count!=0 means that an error occured in d pairs, we should increase the time by bt for the process

    #init time
    dtet=nfails*bt
    #print(tqec+tme_s,nfails,dtet)

    dtet=dtet+tme_s+tqec #total time for storage for this iteration of memory
...
def Scheduler(i,j,tf,dist): #i: number of cycles in QMEM, j is the sequential data identifier
    ...
    tqec=i*t2cyc+tM #in memory ith QEC 
    tme_s=t2cyc+t2cyc*3+t2cyc #merge+split time 
    #ttrav #travelling time

   # t2cyc=(tM+2*tCX+2*tH)*2+(tM+2*tCX)*2
    #t1=t1cyc+t1cyc*3+ttrav+t1cyc+t1cyc*j+tM
    ...
    if(tme_s>=tqec):
        ...
        tf=j*tme_s+ttrav+tqec #sum always first part
    ...
    if(tme_s<tqec):
        ...
        if(tf==0):
            ...
            tf=tf+tqec #sum always the last part


    return tf

def Rate2(store_t,lamb,m):
    ...
    data=10000 #Amount of bell pairs
    Count=0
    Count1=0
    Count_T=[]
    D=[]
    Count_tot=0
    #Changing distance, calculate fiber loss + readout loss for 9 data qubits to be measured correctly
    for dist in range(1000,10000,1000):
        tf=0
        for j in range(data):

            #Do we detect a photon in transmission-receiver?
            ncycles=0
            nfails=0

            while (ncycles!=3): #until d=3 cycles with no interuption are made

                for k in range(3): #in each loss we consider two photons from left and right side (A and B).
                    Count=Fiber_loss(dist*pow(10,-3),lamb,k,j,m)+Count
            ...
            dt,tf=Scheduler2_re(nfails+ncycles,4,j,tf,dist) #tf total time referece, dt local time for storage of operations
            nfails=0

            #If every d boundary bell pairs reaches during d cycles A and B, then we can analyze Count1 to see if we recover all the data qubits
            Count1=0
            store_t=dt
            #Do we recover back the data qubits from the quantum memories to decode the state?
            for k in range(9):

                Count1= Read_loss(store_t*pow(10,-9),store_t*pow(10,-9),k,j,m)+Count1

            T_vec.append(tf)          #Append times of when the classical output is given
                    
            #If there are no occurances of error in any of the 9 qubits pairs during storage you can count in
            if (Count1 == 0):
        ...
        tf=tf*0.000001 #mus into s
        T_vec2.append(T_vec)
        Count_T.append(Count_tot/tf) #rate of bell state
        Count_tot=0
        tf=0
        T_vec=[]
        D.append(dist)
...
def Rate(store_t,lamb,m):
    ...
    data=100000 #Amount of bell pairs
    Count=0
    Count1=0
    Count_T=[]
    D=[]
    ...
    for dist in range(1000,10000,1000):
        for j in range(data):
            ...
            #Do we detect a photon in transmission-receiver?
            for k in range(9): #in each loss we consider two photons from left and right side (A and B).
                Count=Fiber_loss(dist*pow(10,-3),lamb,k,j,m)+Count
            ...
            tf=Scheduler(4,j,tf,dist) #4 iteration cycles
            T_vec.append(tf)                     #Append times of when the classical output is given
                    
            #If there are no occurances of error in any of the 9 qubits you can count in
            if (Count1 == 0):
                Count_tot=Count_tot+1
            Count=0
            Count1=0
    ...
    #Count_T2=[]
    #Count_tot2=0
    #for dist in range(1000,2000,900):
    #    D.append(dist)
    #    print(dist)
    #    for j in range(data):
    #        for k in range(1):
 ...
 #       Count_T2.append(10*np.log10(Count_tot2/data))
  #      Count_tot2=0
   ...
   # print(Count_T,Count_T2)
    return Count_T,D#,Count_T2, D

def Fiber_loss(dist,lamb,k,j,m):
    #We implement this by losing the qubits
    #Measuring the state and disapearing from a memory, how do you inform Alice about that, if this information is secret
    #dist = [km]
    #lamb = [db/km]
...
def test_make_bacon_shor_xx_lattice_surgery_circuit(width: int, height: int, basis: str, rounds: int,noiset: int):
    ...
    expected_distance = min(width // 2, rounds) if basis == 'X' else height // 2
    actual_distance = len(circuit.shortest_graphlike_error())
    #print(expected_distance,actual_distance)
    #assert actual_distance == expected_distance
...
if __name__ == '__main__':
    ...
    vel=c/n 
    ttrav=(dist/vel)*1000000 #photon delay time
    lambd=0.17 #constant for fiber optics

    iterations=5 ############################################################## Amount of QEC iterations
    init=1 #off set
    ...
    ID_err=Noise_model(ttrav,l_p,t_p) #noise from travelling (decoherence and dephasing)
    CX_err,H_err,Loc_H,Loc_CNOT,Read_err,M_err,p_l=Noise_model2(ttrav,l_p,t_p) #gate noise from IBM calibration 
    ...
    ###################################################################
    ############# Rate calculation and loss - delete the identation####
...
#    ax = fig.add_subplot(111)
   ...
   # print(circuit.diagram)
    ...
    #t2cyc=2*tH+4*tCX+tM
   # t2cyc=(tM+2*tCX+2*tH)*2+(tM+2*tCX)*2 #BS code
   ...
   #     rect = [0.35,0.1,0.6,0.6]
    #for l in range(6760,6800,100):
   #     Count,D=Rate(store_t,lambd,m)
   #     print("Count:")
   #     print(Count)
   #     Xl=D
   #     print(D)
   ...
   #     ax.plot(Xl,Count,label='S[[18,2,2]]')
        #ax.plot(Xl,Count2,"--",label='Unencoded Bell pair, $t_{store}:$'+str(store_t/1000)+r'$\mu$s')

   # ax.set_xlabel("Distance [km]")
   # ax.set_ylabel("Protocol sucess rate [kHz]")
   # ax.grid(which='major')
   ...
   # plt.show()
    ...
    #Variables for storage time (X) and Logical (Y) values vectors and their standard deviations in upper and lower bounds
    Vec_Y=[]
    ...
    ### Time complexity calculation based on the amount of iterations 
    #################################################################
    ...
    for j in range(init,iterations):
        ...
        t2cyc=2*tH+4*tCX+tM
        t2=t2cyc+t2cyc*3+ttrav+t2cyc+t2cyc*j+tM
        t3=t2cyc+t2cyc*3+ttrav+t2cyc+t2cyc*j+tM

        print(t1cyc+t1cyc*3+ttrav+t1cyc)
        print(t2cyc+t2cyc*3+ttrav+t2cyc)

        print(" ")
        print("4*tcycle, tmerge+tsplit, t travv,t total")
        print(4*t2cyc+tM, t2cyc+t2cyc*3+t2cyc,ttrav,t2cyc+t2cyc*3+ttrav+t2cyc+t2cyc*1+tM)
        print(" ")
        

        
        
        ID_err2=Noise_model(wait+ttrav,l_p,t_p)
...
""")
        ...
        print("Distances:=====================================================")
        actual_distance1 = len(circuit1.shortest_graphlike_error())
        actual_distance2 = len(circsurf_notr.shortest_graphlike_error())
        actual_distance3 = len(circsurf_notr2.shortest_graphlike_error())


        a=count_determined_measurements_in_circuit(circuit1)
        expected_determined = circuit1.num_detectors + circuit1.num_observables
        print(expected_determined,a)
